...
// --- strain invariants ---
console.log('Checking strains.ts…');
const strainsSource = readFileSync(join(root, 'src/constants/strains.ts'), 'utf8');
const strainBlocks = extractObjectBlocks(strainsSource);
const strainIds = new Set();
const strainRequired = ['name', 'thc', 'cbd', 'description', 'effects', 'terpenes', 'genetics', 'breeder'];
const strainPaired = ['description', 'effects', 'medicalEffects', 'terpenes'];

if (strainBlocks.length === 0) {
  fail('No strain objects found in src/constants/strains.ts');
}

for (const block of strainBlocks) {
  if (strainIds.has(block.id)) {
    fail(`Duplicate strain id: ${block.id}`);
  } else {
    strainIds.add(block.id);
  }

  for (const field of strainRequired) {
    if (!hasNamedField(block.text, field)) {
      fail(`Strain ${block.id} is missing ${field}`);
    }
  }

  for (const field of strainPaired) {
    if (hasNamedField(block.text, field) && !hasNamedField(block.text, `${field}_en`)) {
      fail(`Strain ${block.id} is missing ${field}_en`);
    }
  }
  ...
  if (indica == null || sativa == null) {
    fail(`Strain ${block.id} is missing indica/sativa`);
  } else if (indica + sativa !== 100) {
    fail(`Strain ${block.id} indica (${indica}) + sativa (${sativa}) !== 100`);
  }
}
if (strainBlocks.length > 0) {
  console.log(`  OK — ${strainBlocks.length} strains`);
}
